...
wui_blks <- st_read("data/shapes/all_blocks_wui_demo1.geojson")
```
...
or works in the WUI or not. To do this, let's create an sf object with 
five variables: whether the block was in the WUI in 2020, whether the block was 
intermix in 2020, whether the block was interface in 2020, whether the block was
new WUI (extensive growth) in 2020, and block ID. We'll merge those variables 
into our datasets of surveyor locations

```{r}
wui_status20 <- wui_blks %>%
  select(BLK20, wui20, intermix20, interfac20, wui_new20)
```

Let's join points to blocks. This will tell us which points are assigned to
which blocks and their associated WUI status.
```{r}
places_to_c_point2 <- st_join(places_to_c_point1, wui_status20)
places_to_p_point2 <- st_join(places_to_p_point1, wui_status20)
places_to_w_point2 <- st_join(places_to_w_point1, wui_status20)
```
...
addresses to work addresses.
```{r}
cp_addresses <- inner_join(p_addresses, as.data.frame(c_addresses), by = 'id')
cw_addresses <- inner_join(w_addresses, as.data.frame(c_addresses), by = 'id')
```
...
```{r}
st_write(c_addresses, "data/shapes/current_addresses_07-18-24.geojson")
st_write(cp_addresses, "data/shapes/previous_addresses_07-18-24.geojson")
st_write(cw_addresses, "data/shapes/work_addresses_07-18-24.geojson")
st_write(point_to_blk1, "data/shapes/survey_blk_07-18-24.geojson")
st_write(point_to_bg1, "data/shapes/survey_bg_07-18-24.geojson")
st_write(point_to_pl1, "data/shapes/survey_pl_07-18-24.geojson")
```
...
c_addresses1 <- c_addresses %>%
  select(id, c_partial, county, c_cnty_dist, c_inside_cnty, place, c_pl_dist, 
         c_pl_res, wui20, intermix20, interfac20, wui_new20, wui_bg, lint_hu,
         hint_hu, lint_pop, hint_pop) %>%
  st_drop_geometry()

cp_addresses1 <- cp_addresses %>%
  select(id, p_partial, p_county, p_cnty_dist, p_inside_cnty, p_place, p_pl_dist, 
         p_pl_res, p_wui20, p_intermix20, p_interfac20, p_wui_new20, p_wui_bg, p_lint_hu,
         p_hint_hu, p_lint_pop, p_hint_pop) %>%
  st_drop_geometry()

cw_addresses1 <- cw_addresses %>%
  select(id, w_partial, w_county, w_cnty_dist, w_inside_cnty, w_place, w_pl_dist, 
         pl_work, w_wui20, w_intermix20, w_interfac20, w_wui_new20, w_wui_bg, w_lint_hu,
         w_hint_hu, w_lint_pop, w_hint_pop) %>%
  st_drop_geometry()
```
